...
maxMinWords.max = (labelSelector, selector, measure, max) => {
  $(selector).keyup(function (e) {
    ...
    if (wordCount >= max && !isSpace(inputValue)) {
      e.preventDefault();
      e.target.value = truncateText(inputValue, max);
    }
...
function getWordsLength(editorSelector) {
  return $(editorSelector).val()
    ? $(editorSelector).val().trim().split(/\s+/).length
    : 0;
}

function setLabelText(labelSelector, editorSelector, measure, count) {
  $(labelSelector).text(
    `${measure} of ${count} words (${getWordsLength(editorSelector)}/${count})`
  );
}

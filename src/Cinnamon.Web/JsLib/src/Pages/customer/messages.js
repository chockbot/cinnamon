...
messages.initControls = (obj, elementId) => {
    ...
    $(document).off('keyup').on("keyup", addMembersInput, function (e) {
        e.preventDefault();
        e.stopPropagation();
        dotnetObj.invokeMethodAsync('HandleInputAddMember', $(addMembersInput).text());
    });

    $(document).off('keyup').on("keyup", chatRoomSearchInput, function (e) {
        e.preventDefault();
        e.stopPropagation();

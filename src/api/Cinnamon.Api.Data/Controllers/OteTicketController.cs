...
public class OteTicketController : ControllerBase 
{
    ...
    public async Task<IActionResult> GetByActivityId(int activityId, [FromQuery] GetByActivityIdArgs args)
    {
        try
        {
            var result = await oteTicketRepository.GetByActivityId(activityId, args.SearchValue ?? string.Empty, args.CountPerPage, (args.PageIndex - 1) * args.CountPerPage, args.IncludeCustomer ?? false, args.IncludeImageAsResult ?? false);
            ...
            var all = await oteTicketRepository.GetByActivityId(activityId, args.SearchValue ?? string.Empty, null , null, args.IncludeCustomer ?? false, args.IncludeImageAsResult ?? false);

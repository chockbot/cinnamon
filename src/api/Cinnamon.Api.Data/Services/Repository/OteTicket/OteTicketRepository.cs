...
public class OteTicketRepository : IOteTicketRepository
{
    ...
    public async Task<AppResult<IEnumerable<OteTicketDTO>>> GetByActivityId(int activityId, int dateId, int? count, int? skip, 
        bool includeCustomer = false, bool includeImageAsResult = false)
    {
        try
        {
            var result = await dataStore.OteTicket.GetByActivityId(activityId, dateId, count, skip, includeCustomer, includeImageAsResult);
            if(!result.Succeeded || result.Result is null)
            {
                return AppResult<IEnumerable<OteTicketDTO>>.CreateFailed(new ApplicationException(result.Message), result.Message);
            }
